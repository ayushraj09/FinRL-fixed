...
from finrl.meta.data_processors.processor_indian_api import IndianAPIProcessor as IndianAPI
...
class DataProcessor:
    def __init__(self, data_source, tech_indicator=None, vix=None, **kwargs):
        ...
        elif data_source == "indian_api":
            try:
                api_key = kwargs.get("api_key") or kwargs.get("API_KEY")
                base_url = kwargs.get("base_url", "https://stock.indianapi.in")
                
                # Remove conflicting parameters to avoid duplicate arguments
                filtered_kwargs = {k: v for k, v in kwargs.items() 
                                 if k not in ['api_key', 'API_KEY', 'base_url']}
                
                self.processor = IndianAPI(api_key=api_key, base_url=base_url, **filtered_kwargs)
                print("Indian Stock API successfully connected")
            except Exception as e:
                raise ValueError(f"Please provide correct Indian API credentials: {e}")
